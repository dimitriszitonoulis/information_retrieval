...
def find_nearest_neighbors(
...
) -> List[Tuple[List[NDArray], int]]:
    """
    Finds the elements that are the closest to
    the query.
    This is done by getting the centroids that are the
    ...
    for i in range(queries.shape[0]):
        query_vector: NDArray[float32] = queries[i]
        ...
        neighbors, n_distances_calculated = _find_nearest_neighbors(
            query_vector=query_vector,
        ...
        results.append((neighbors, n_distances_calculated))
...
def _find_nearest_neighbors(
    ...
    for centroid_idx in neighboring_centroids_indices:
        ...
        nearest_members_indices = nearest_members_indices[0]

        candidate_vectors: NDArray[float32] = matrix[nearest_members_indices]
